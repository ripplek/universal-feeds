...
- Via the reach layer (OpenCLI browser bridge, desktop-only).
  - `feed` — personalized homepage recommendations (no query)
  - `search` — topic query
- Enable per config under `platforms.youtube.reach`; see `docs/REACH.md`.
- Notes: reuses your logged-in Chrome session; not available headless/CI.
